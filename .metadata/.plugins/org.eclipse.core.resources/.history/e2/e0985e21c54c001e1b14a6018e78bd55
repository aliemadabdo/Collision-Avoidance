...
#elif TIM0_MODE == PWM_PHASE
	if(Copy_u8OC0PinAction == TIM0_OC0_SET_UPC_CLR_DNC){
		SET_BIT(TCCR0, COM00);
		SET_BIT(TCCR0, COM01);
		OCR0 = (255 -(((Copy_u8DutyCycle/100.0)*510)/2));
	}
	else{
		SET_BIT(TCCR0, COM01);
		CLR_BIT(TCCR0, COM00);
		OCR0 = (((Copy_u8DutyCycle/100.0)*510)/2);
	}
...
//
//void __vector_10 (void) __attribute__ ((signal,__INTR_ATTRS)) __VA_ARGS__;
//
